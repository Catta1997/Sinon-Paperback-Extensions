...
var source = (() => {
  ...
  // src/DevTestExtension/main.ts
  var main_exports = {};
  __export(main_exports, {
    DevTestExtension: () => DevTestExtension,
    ScansExtension: () => ScansExtension
  });
  init_buffer();
  ...
  var ScansExtension = class {
    ...
    async getChapters(sourceManga) {
      ...
      const $2 = load(Application.arrayBufferToUTF8String(data2));
      const chapters = [];
      const arrChapters = $2("li.item").toArray().reverse();
      for (const item of arrChapters) {
        const chapterId = $2("a", item).attr("href") ?? "";
        console.log("ID " + chapterId);
        const chapN = $2(item).closest(".item").attr("data-number");
        ...
        chapters.push({
          chapterId: chapterId.match(/manga\/((.)*\/)/)?.[1] ?? "",
          //`89400-kaoru-hana-wa-rin-to-saku-chapter-${chapNum}`,
          sourceManga,
          langCode: "en",
          chapNum,
          title: `${chapNum}`,
  ...
  var DevTestExtension = new ScansExtension();
