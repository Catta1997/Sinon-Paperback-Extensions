...
import { type Metadata, type SearchMetadata } from "./models";

export class Parser {
  private async getCheerio(url: string, init?: RequestInit) {
    const req = init ? (init as Request) : ({ url: url, method: "GET" } as Request);
  ...
  async parseSearch(
    ...
    form.append("keyword", query.title ?? "");
...
*/
  ...
  async parseChapters(sourceManga: SourceManga): Promise<Chapter[]> {
    ...
    $("ul.chapter-chs > li > a").each((_, el) => {
      const chapterId = $(el).attr("href")?.replace(BASE_URL, "");
      const title = $(el).text().trim();
      chapters.push({
        chapNum: 1,
        langCode: "en",
        chapterId: chapterId ?? "AAAAA",
