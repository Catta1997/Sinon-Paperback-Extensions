...
export class Parser {
  ...
  async parseMangaDetails(mangaId: string): Promise<SourceManga> {
    ...
    const text = $(".mdc-typography--caption.site-manga-info__title-text").text();
  ...
  parseMangaCard(html: string): MangaCardInfo {
    ...
    const coverStyle = root.find(".site-manga-card__media--japanese-b6").attr("style") || "";
    const coverImageMatch = coverStyle.match(/url\(["']?(.*?)["']?\)/);
    const coverImage = coverImageMatch ? coverImageMatch[1] : undefined;
    const title = root.find(".site-manga-card__title--primary").text().trim() || undefined;
    const subtitle = root.find(".site-manga-card__title--secondary").text().trim() || undefined;
