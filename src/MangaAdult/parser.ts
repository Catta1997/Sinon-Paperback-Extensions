...
export class Parser {
    ...
    parseInTendenzaMese(
        $: any,
    ): [{ items: DiscoverSectionItem[] }, { items: DiscoverSectionItem[] }] {
    ...
    getDate(dataString: string): Date {
        ...
        if (parts.length > 3)
            return new Date(oggi.getFullYear(), oggi.getMonth(), oggi.getDay()); // Controlla che ci siano esattamente due elementi
    ...
    parseLastAddedSetcion($: any): Promise<PagedResults<DiscoverSectionItem>> {
        ...
        for (const obj of arrLatest) {
            ...
            const addedDate = $("i.ml-auto.mt-auto", obj)
                .first()
                .text()
                .trimEnd();
            latest.push({
